...
def _ws_sign(secret: str) -> tuple:
    ...
    nonce = ts   # BloFin WS docs show nonce = timestamp for login
    path  = "/users/self/verify"
    method = "GET"
    msg   = f"{path}{method}{ts}{nonce}"
    hex_sig = hmac.new(secret.encode(), msg.encode(), hashlib.sha256).hexdigest()
        ...
        while self._running:
            try:
                ...
                async with websockets.connect(
                    ...
                    # Authenticate
                    ts, nonce, sig = _ws_sign(BLOFIN_API_SECRET)
